...
component output="false" persistent="false" trigger="" {
	...
	function dependency(required artifactId, required groupId, required version, type="zip", classifier="", mapping="", exclusions="")  {
		var params = {};
		if(mapping == "") {
			var _classifier = classifier != "" ? "-" & classifier : "";
			var slashGroupId = replace(groupId,".","/","all");
			var artifactPath = "/#slashGroupId#/#artifactId#/#version#/#artifactId#-#version##_classifier#.#type#";
			...
			var localDir = "#shell.getArtifactsDir()#/#slashGroupId#/#artifactId#/#version#/";
			directoryExists(localDir) ? "" : directoryCreate(localDir,true);
